...
class DepartmentController extends Controller
{
    public function index()
    {
        try {
            ...
            return response()->json(['success' => false, 'message' => 'Failed to fetch departments: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            // ✅ B5: reject whitespace-only after trim
            if ($name === '' || $code === '') {
                return response()->json([
                    'success' => false,
                    'message' => 'Validation failed',
            ...
            // ✅ B4: notify Budget Office users
            $budgetOfficeUsers = User::where('role', 'budget_office')
                ->where('status', 'active')
                ->get();

            foreach ($budgetOfficeUsers as $bo) {
                try {
                    ...
                    Log::warning('Failed to notify budget_office user ' . $bo->user_id . ': ' . $e->getMessage());
                }
            }
    ...
    public function destroy($id)
    {
        try {
            ...
            // ✅ Also check for existing trip tickets
            if (\App\Models\TripTicket::where('department_id', $id)->exists()) {
                return response()->json([
                    'success' => false,
    ...
    public function toggleStatus(Request $request, $id)
    {
        try {
            ...
            return response()->json(['success' => false, 'message' => 'Failed to update department status: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function getActiveDepartments()
    {
        try {
            ...
            return response()->json(['success' => false, 'message' => 'Failed to fetch active departments: ' . $e->getMessage()], 500);
        }
    }
