...
import { gsoAPI, userAPI, vehicleAPI, departmentAPI, gpsAPI, auditAPI } from '../../services/api';
import {
  LayoutDashboard,
  Clock,
  CheckCircle,
...
// FIXED: formatTime function that shows "Just now", "5 minutes ago", etc.
...
//  Keep formatDateTime for other uses if needed
...
// ============================================
//  GSO REAL-TIME NOTIFICATIONS
// ============================================


const setupGsoRealtimeNotifications = (user, fetchAllDataRef, setForceUpdate) => {
  if (!user) return;

  console.log('🔔 Setting up GSO real-time notifications for user:', user.user_id);

  if (!echo.connector || !echo.connector.pusher) {
    console.warn('⚠️ Echo connector not available, retrying in 2s...');
    setTimeout(() => setupGsoRealtimeNotifications(user, fetchAllDataRef, setForceUpdate), 2000);
    return;
  }
  ...
  const subscribeToPrivateChannel = () => {
    try {
      ...
      channel.subscribed(() => {
        console.log(`✅ Subscribed to private notifications.${user.user_id}`);
      });
  ...
  const handleNotificationByType = (data) => {
    ...
    switch (type) {
      ...
      case 'trip_completed':
        console.log('🏁 Trip completed - refreshing data');
        setTimeout(() => {
          fetchAllDataRef.current?.();
          setForceUpdate(prev => prev + 1);
        }, refreshDelay);
      ...
      case 'trip_created':
        console.log('📋 New trip created - refreshing data');
        setTimeout(() => {
          fetchAllDataRef.current?.();
          setForceUpdate(prev => prev + 1);
        }, refreshDelay);
  ...
  if (!activeTrips || activeTrips.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
        ...
        <p className="text-slate-500 dark:text-slate-400">No active trips with GPS data</p>
        <p className="text-sm text-slate-400 dark:text-slate-500">Active trips with GPS tracking will appear here</p>
      </div>
    );
  }
  ...
  if (tripsWithLocation.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
        ...
        <p className="text-sm text-slate-400 dark:text-slate-500">Waiting for drivers to share location</p>
      </div>
    );
  }
                ...
                <Popup>
                  <div className="p-2 min-w-[200px]">
                    ...
                    <div className="space-y-1 text-sm">
                      <p><span className="text-slate-500">Vehicle:</span> {trip.vehicle?.plate_number}</p>
                      <p><span className="text-slate-500">Driver:</span> {trip.driver?.name}</p>
                      <p><span className="text-slate-500">Destination:</span> {trip.destination}</p>
                      <p><span className="text-slate-500">Speed:</span> {current_location.speed_kmh || 0} km/h</p>
                      <p className="text-xs text-slate-400">
                        Updated: {new Date(current_location.recorded_at).toLocaleTimeString()}
                      </p>
                    </div>
                    <div className="mt-2 flex gap-2">
                      <button
                        onClick={() => window.location.href = `/gso/trip/${trip.trip_id}`}
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      ...
                      {/* ✅ FIXED: Using formatTime instead of formatDateTime */}
...
const GsoDashboard = () => {
  ...
  // ============================================
  // ✅ AUDIT LOG QUERY - Auto-refresh every 10 seconds
  // ============================================
  ...
  }, [allTrips, pendingTickets, users, vehicles, departments, activeTrips.length, navigate]);
...
useEffect(() => {
  ...
  if (user) {
    setupGsoRealtimeNotifications(user, fetchAllDataRef, setForceUpdate);
  }

  // ✅ Cleanup on unmount - leave channels
  return () => {
    try {
      if (user) {
...
const TicketTable = ({ 
  ...
  // ✅ Helper to get department name from various sources
  const getDepartmentName = (ticket) => {
    // Try different possible locations for department name
    if (ticket?.department_name) return ticket.department_name;
    if (ticket?.department?.department_name) return ticket.department.department_name;
    if (ticket?.department?.name) return ticket.department.name;
    if (ticket?.dept_name) return ticket.dept_name;
    if (ticket?.charged_to_department) return ticket.charged_to_department;
    if (ticket?.charge_to_department) return ticket.charge_to_department;
    if (ticket?.charge_to) {
      // If charge_to is a department code, try to find the name
      const dept = departments?.find(d => d.department_code === ticket.charge_to);
      if (dept) return dept.department_name;
    }
    
    // If department_id is available, try to match with departments list
    const deptId = ticket?.department_id || ticket?.department?.department_id;
    if (deptId && departmentMap) {
      return departmentMap[deptId] || 'N/A';
    }
      ...
      <Table>
        ...
        <TableBody>
          {tickets.map((ticket, index) => {
            ...
            const deptName = getDepartmentName(ticket);
            return (
              <TableRow 
                key={ticketId || index} 
                className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
  ...
  return (
    <div className="space-y-6 p-6 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 min-h-screen">
      ...
      <Card className="dark:bg-slate-800/80 dark:border-slate-700">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="dark:text-slate-400">
                ...
                {gpsFetching && <span className="ml-2 text-blue-500">(Updating...)</span>}
              </CardDescription>
            </div>
            <div className="flex gap-2">
              ...
              >
                {/* <RefreshCw className={`h-4 w-4 mr-1 ${gpsFetching ? 'animate-spin' : ''}`} />
                Refresh */}
              </Button>
              <Button
                variant="outline"
        ...
        {showLiveTracking && (
          <CardContent>
            <LiveTrackingMap activeTrips={activeTrips} loading={gpsLoading} />
          </CardContent>
        )}
      </Card>
...
<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
  ...
  <TabsContent value="all" className="space-y-4 mt-6">
    <Card className="dark:bg-slate-800/80 dark:border-slate-700">
      ...
      {/* ✅ AUDIT LOG SECTION - Toggle with Chevron */}
      <Card className="dark:bg-slate-800/80 dark:border-slate-700">
        <CardHeader 
